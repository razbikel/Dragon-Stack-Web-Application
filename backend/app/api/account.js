...
router.post('/signup', (req, res, next) => {
    ...
    .then(({account}) => {
        if (!account){
            ...
            const error = new Error('This username has alredy been taken');
...
router.get('/like-dragon',(req, res, next) => {
    const dragonId = req.query.dragonId;
    const sessionString = req.cookies.sessionString;

    authenticatedAccount({ sessionString })
    .then(({ account }) => {
        return AccountLikesTable.isAccountLikedDragon({ accountId: account.id, dragonId })
    })
    .then(({ dragonId }) => {
